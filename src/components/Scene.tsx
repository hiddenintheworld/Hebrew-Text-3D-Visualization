...
function addTextMeshesToScene(text: string, scene: THREE.Scene, font: Font, gridSize: GridSize, spacing: number) {
  scene.children.slice().forEach(child => scene.remove(child)); // Remove previous meshes

  const maxChars = gridSize.width * gridSize.height * gridSize.depth;
  for (let i = 0; i < Math.min(text.length, maxChars); i++) {
    const letter = text[i];
    const color = letterColors[letter] || '#FFFFFF'; // Default color if no specific color found
      ...
      loadTextData('/data/genesis_processed_data.txt')
        .then(text => {
          if (text) {
            ...
            addTextMeshesToScene(letterString, scene, font, { width: 7, height: 7, depth: 7 }, 1.5);
            animate();
          }
        });
    });
    ...
    importGUI().then((GUI) => {
      ...
      const updateScene = (params: { width: number, height: number, depth: number, spacing: number }) => {
        if (scene && fontRef.current) {
          addTextMeshesToScene(textDataRef.current, scene, fontRef.current, { width: params.width, height: params.height, depth: params.depth }, params.spacing);
        }
      };
    });
  }, []);
